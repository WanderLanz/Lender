...
impl<L: DoubleEndedFallibleLender> DoubleEndedFallibleLender for &mut L {
    fn next_back(&mut self) -> Result<Option<FallibleLend<'_, Self>>, Self::Error> { (**self).next_back() }
    fn advance_back_by(&mut self, n: usize) -> Result<Option<NonZeroUsize>, Self::Error> { (**self).advance_back_by(n) }
    fn nth_back(&mut self, n: usize) -> Result<Option<FallibleLend<'_, Self>>, Self::Error> { (**self).nth_back(n) }
}
