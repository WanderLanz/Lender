...
impl<L: FallibleLender> FallibleLender for &mut L {
    type Error = L::Error;
    #[inline]
    fn next(&mut self) -> Result<Option<FallibleLend<'_, Self>>, Self::Error> { (**self).next() }
    #[inline]
    fn size_hint(&self) -> (usize, Option<usize>) { (**self).size_hint() }
    #[inline]
    fn advance_by(&mut self, n: usize) -> Result<Option<NonZeroUsize>, Self::Error> { (**self).advance_by(n) }
}
