...
where
    T: Try,
{
    fn from_residual(residual: Result<T::Residual, E>) -> Self { Self(residual.map(T::from_residual)) }
}
    ...
    T: Try,
{
    ...
    fn from_output(output: Self::Output) -> Self { Self(Ok(T::from_output(output))) }
...
impl<E, L> IntoFallible<E, L> {
    pub(crate) fn new(lender: L) -> Self { Self { lender, _marker: PhantomData } }
    ...
    L: Lender,
{
    ...
    #[inline]
    fn next(&mut self) -> Result<Option<FallibleLend<'_, Self>>, Self::Error> { Ok(self.lender.next()) }
...
impl<E, L: Lender> From<L> for IntoFallible<E, L> {
    fn from(lender: L) -> Self { Self::new(lender) }
}
...
{
    #[inline]
    fn next_back(&mut self) -> Result<Option<FallibleLend<'_, Self>>, Self::Error> { Ok(self.lender.next_back()) }
