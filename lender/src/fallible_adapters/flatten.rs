    ...
    for<'all> FallibleLend<'all, L>: IntoFallibleLender,
{
    ...
    pub fn into_inner(self) -> L { *self.inner.lender }
}
    ...
    for<'all> FallibleLend<'all, L>: IntoFallibleLender<Error = L::Error>,
{
    ...
    #[inline]
    fn next(&mut self) -> Result<Option<FallibleLend<'_, Self>>, Self::Error> { self.inner.next() }
    #[inline]
...
where
    for<'all> FallibleLend<'all, L>: IntoFallibleLender,
{
    pub(crate) fn new(lender: L) -> Self { Self { lender: Box::new(lender), inner: None } }
}
...
mod test {
    ...
    impl<'a, 'lend> FallibleLender for Child<'a> {
        ...
        fn next(&mut self) -> Result<Option<FallibleLend<'_, Self>>, Self::Error> { Ok(Some(&self.array_ref)) }
    }
