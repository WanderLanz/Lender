    ...
    L: for<'all> FallibleLending<'all, Lend = &'all T>,
{
    ...
    type Error = L::Error;
    #[inline]
    fn next(&mut self) -> Result<Option<Self::Item>, Self::Error> { self.lender.next().map(Option::<&T>::copied) }
    #[inline]
    fn size_hint(&self) -> (usize, Option<usize>) { self.lender.size_hint() }
}
...
{
    #[inline]
    fn next_back(&mut self) -> Result<Option<Self::Item>, Self::Error> { self.lender.next_back().map(Option::<&T>::copied) }
}
